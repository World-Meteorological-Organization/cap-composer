...
def setup_logging():
    ...
    logger.add(
        sys.stderr, format=loguru_format, level=settings.CAP_COMPOSER_LOG_LEVEL
    )
...
def setup_telemetry(add_django_instrumentation: bool):
    """
    Sets up logging and when the env var CAP_COMPOSER_ENABLE_OTEL is set to any non-blank
    ...
    if otel_is_enabled():
        ...
        print("Not configuring telemetry due to CAP_COMPOSER_ENABLE_OTEL not being set.")
...
def _setup_log_exporting(logger):
    ...
    handler = LogGuruCompatibleLoggerHandler(
        level=settings.CAP_COMPOSER_LOG_LEVEL,
        logger_provider=logger_provider,
    )
    logger_provider.add_log_record_processor(BatchLogRecordProcessor(exporter))
    logger.add(handler, format="{message}", level=settings.CAP_COMPOSER_LOG_LEVEL)
